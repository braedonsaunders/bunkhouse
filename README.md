...
**Open-source AI employees for main-street business.** Hire hands that live in your inbox.

A bunkhouse is where the hired hands live. This one is where your AI hands live: each one
...
## What makes a hand different from a chatbot
...
- **A job, not a prompt.** Hands are hired from role packs — office administrator,
...
- **Your company's doctrine, enforced.** Upload the employee handbook, price sheets,
  warranty policy, and SOPs. Procedures become versioned, governed objects every hand cites
...
- **A mixed org chart.** The directory holds your human employees too. Hands route work to
  the right person by title, ask humans questions over email, delegate to each other, and
  escalate to their manager.
- **A desk you can look over.** The observatory shows each hand at work — the browser
  ...
  replayable.
- **A salary, not a meter.** Each hand's salary is its monthly model-token budget, paid to
  your own API keys — bring any provider, run different models per hand. Overage is overtime.
  Bunkhouse takes nothing.
- **Memory you can read.** Per-hand memory is human-readable notes you can open, correct,
  and delete from the hand's HR profile, plus a governed company-knowledge layer.
...
- `apps/web` — the HR app: directory, hand profiles, org chart, hiring, observatory.
- `packages/runtime` — the model-agnostic employee loop: provider seam (any LLM per hand),
