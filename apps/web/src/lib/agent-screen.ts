 ...
 * that publishes the track into the call's room, the browser session that
 * feeds it frames, and the call page that renders it. It is deliberately
 ...
 * room; publishing what its browser sees costs no new transport, no new
 * infrastructure, and no polling at all.
 *
 * The screenshot ledger is untouched by any of this. It is the audit record —
 * doctrine #9 — and it is what the run desk replays afterwards. The track is
 * the live view; the ledger is the evidence.
 */

/**
 * The LiveKit track name the agent's browser is published under.
 *
 * The track's *source* is ScreenShare, chosen deliberately: it genuinely is a
 * screen being shared, and a server that thinks a 1280-wide page of small text
 * is a camera will degrade it like a talking head — resolution first — which is
 * exactly backwards for something whose whole content is legible text.
 *
 ...
 * agent's page and never a guest's desktop.
 */
export const AGENT_SCREEN_TRACK_NAME = 'agent-browser'

/**
 * The size the agent's browser runs at, and therefore the size the track is
 * published at: one number, so the screencast never rescales in the ordinary
 * case and the published frame is the page pixel for pixel. 64:45 — the call
 * stage's own aspect, so the page fills the stage without letterboxing.
 */
 ...
 * while a still page — which is what a browser shows most of the time — costs
 * nothing at all, because Chromium only emits a screencast frame when
