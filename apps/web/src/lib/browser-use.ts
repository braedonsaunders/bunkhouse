...
import { browserSessions, browserSteps, people, type BrowserStepDetail } from '../db/schema'
...
/**
 * Computer use: the agent drives a real headless browser — the ability for
 * work that a plain page fetch cannot do (sign in, click through, fill a form,
 * read what only renders with JavaScript).
 *
 * Doctrine: gated abilities are recorded. Every step screenshots the viewport,
 * files it in the files ledger, and appends a browser_steps row, so an operator
 * can replay the whole visit from the run desk. Recording is not optional —
 * a step that cannot be recorded is still written to the ledger with the
 * reason its frame is missing.
 *
...
function resolveExecutable(): string | null {
  ...
  // environment-provided path; Chromium reports its own launch error when the
...
/**
 * Fail-closed capability detection: with no Chromium on the box the browser
 * abilities are not offered at all, exactly like the sandboxed shell.
 */
...
type LiveSession = {
  ...
  browser: Browser
  page: Page
  /** Steps already on the ledger for this session; the next step is seq + 1. */
  seq: number
...
/** The run's session row — created on first use, reused for the run's lifetime. */
async function sessionRow(args: {
  tenantId: string
  personId: string
  runId: string
  ...
  return app.withTenant(args.tenantId, async () => {
    const [existing] = await app.db
      ...
      .where(and(eq(browserSessions.tenantId, args.tenantId), eq(browserSessions.runId, args.runId)))
    if (existing) {
      const [last] = await app.db
        .select({ seq: browserSteps.seq })
        .from(browserSteps)
        .where(and(eq(browserSteps.tenantId, args.tenantId), eq(browserSteps.sessionId, existing.id)))
    ...
    const [created] = await app.db
      ...
      .values({ tenantId: args.tenantId, personId: args.personId, runId: args.runId, status: 'active' })
      .returning({ id: browserSessions.id })
    if (!created) throw new Error('The browser session could not be recorded.')
...
async function openSession(args: { tenantId: string; person: PersonRow; runId: string }): Promise<LiveSession> {
  ...
  if (!executable) throw new Error('No browser is installed on this host.')
  const row = await sessionRow({ tenantId: args.tenantId, personId: args.person.id, runId: args.runId })
  ...
  try {
    ...
    // The browser writes nothing to disk: downloads are refused outright.
    ...
    await markSessionStatus(args.tenantId, row.id, 'failed')
    throw error
  }
}
...
/**
 * End the run's browser session: close the page and browser, stamp the row.
 * Idempotent — the run teardown, the agent's own browser_close, and the idle
 * reaper all land here. Exported so run teardown can guarantee it.
 */
export async function closeBrowserSession(runId: string): Promise<void> {
  ...
  await markSessionStatus(session.tenantId, session.sessionId, 'ended')
}
...
async function recordStep(
  ...
  session.seq = seq
  const recorded: BrowserStepDetail = { ...detail }
  ...
  await app.withTenant(session.tenantId, async () => {
    await app.db.insert(browserSteps).values({
      tenantId: session.tenantId,
 ...
 */
function shownFrame(step: StepRecord, url: string): Record<string, AbilityFrame | string> {
...
/**
 * The agent's browser. Every ability is computer_use — the autonomy dial
 * governs the whole family, and nothing here runs unrecorded.
 */
