...
--> statement-breakpoint
ALTER TABLE "memories" ADD COLUMN "tsv" tsvector GENERATED ALWAYS AS (
...
DROP POLICY IF EXISTS tenant_isolation ON memory_revisions;
DROP POLICY IF EXISTS tenant_write_insert ON memory_revisions;
DROP POLICY IF EXISTS tenant_write_update ON memory_revisions;
DROP POLICY IF EXISTS tenant_write_delete ON memory_revisions;
CREATE POLICY tenant_isolation ON memory_revisions
  USING (tenant_id = nullif(current_setting('app.tenant_id', true), '')::uuid)
  WITH CHECK (tenant_id = nullif(current_setting('app.tenant_id', true), '')::uuid);

ALTER TABLE memory_links ENABLE ROW LEVEL SECURITY;
ALTER TABLE memory_links FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS tenant_isolation ON memory_links;
DROP POLICY IF EXISTS tenant_write_insert ON memory_links;
DROP POLICY IF EXISTS tenant_write_update ON memory_links;
DROP POLICY IF EXISTS tenant_write_delete ON memory_links;
...
DROP POLICY IF EXISTS tenant_isolation ON memory_proposals;
DROP POLICY IF EXISTS tenant_write_insert ON memory_proposals;
DROP POLICY IF EXISTS tenant_write_update ON memory_proposals;
DROP POLICY IF EXISTS tenant_write_delete ON memory_proposals;
CREATE POLICY tenant_isolation ON memory_proposals
  USING (tenant_id = nullif(current_setting('app.tenant_id', true), '')::uuid)
  WITH CHECK (tenant_id = nullif(current_setting('app.tenant_id', true), '')::uuid);
