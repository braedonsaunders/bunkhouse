...
3. **Bi-temporal supersession** (Zep's idea as Postgres rows): notes never edited into
   ...
   snapshot. "What did this agent believe on July 1" is a WHERE clause.
...
6. **Consolidation jobs** (sleep-time compute / Dreaming), all outputs readable:
   nightly per-agent journal (episodes + fact candidates from run events), weekly
...
7. **Promotion agent→company is approval-gated** (memory_proposals) — agents nominate,
...
memory_notes(id, tenant_id, scope agent|company, person_id, slug, kind, title, body_md,
  pinned, importance 1-5, valid_from, valid_until, superseded_by, source_run_id,
  author agent|human|consolidator, last_used_at, use_count, tsv GENERATED, embedding NULLABLE)
