...
Owner's epic: configurable voice per hand; call a hand on a real telephone; hands send
...
- **Cascaded (STT → hand's own model → TTS)**: Deepgram Flux STT <300ms; ElevenLabs
  (~75ms streaming TTS, best cloning: instant clone from ~1min audio) / Cartesia
  (<90ms). ~1–1.5s round trip with streaming — acceptable for phone, and the hand's
  *actual configured model* answers with its full governed toolset.
- **Both shapes ship as adapters.** `voice.mode: 'realtime' | 'cascade'` per hand.
...
2. **The runtime stays the brain.** The voice worker (new `apps/voice-worker`, Node
   ...
   `governedToolSet`; a mid-call `approval` dial answer makes the hand *say* it needs
   sign-off and log the pending approval, same as email runs.
3. **Voice is HR config, not agent config.** The hand's profile gets a Voice section
...
6. **Cost is salary.** Per-call provider + carrier spend computed with the shared
   `money` type, metered into the hand's monthly budget next to tokens; minutes and
   $ visible on the call record and the hand's budget page.
...
call_turns(id, session_id, seq, at, speaker hand|human|system, kind utterance|
...
2. **Inbound phone.** livekit/sip in compose; Settings → Telephony (gate `telephony`,
   ...
   itself; recording, transcript, run all land on the hand's Calls tab.
3. **Outbound phone.** `place_call` ability + `phone_call` autonomy category
   (dial row appears on every hand's autonomy page); approval flow; "Ask to call"
   action from the hand profile and from mail threads; call outcome logged to the run.
4. **Video + screen share.** `send_meeting_link` ability (delivers over mail/SMS —
   ...
   tokenized UI); hand joins as participant, samples screen-share frames (~1fps,
...
5. **Operations polish.** Transfer-to-human (SIP REFER), voicemail→mail-thread
   handoff, per-hand call-minutes budget lines, retention policies UI.
